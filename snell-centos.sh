...
TMP_SCRIPT=$(mktemp)
if curl -sL https://raw.githubusercontent.com/llodys/snell/main/snell-centos.sh -o "$TMP_SCRIPT"; then
    bash "$TMP_SCRIPT"
    rm -f "$TMP_SCRIPT"
else
...
update_script() {
    ...
    # 下载最新版本
    if curl -sL https://raw.githubusercontent.com/llodys/snell/master/snell-centos.sh -o "$TMP_SCRIPT"; then
        # 获取新版本号
